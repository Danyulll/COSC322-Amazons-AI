	...
	public boolean handleGameMessage(String messageType, Map<String, Object> msgDetails) {
		...
		case GameMessage.GAME_ACTION_MOVE:
			...
			if (white) {
				...
				//update board
				board.updateGameBoard(board, QueenPosCurEnemey, QueenPosNewEnemey, ArrowPosEnemey, white);
				Board boardBeforeMove = (Board) board.clone();
				// Construct initial Game tree
				System.out.println("Constructing tree");
				Tree partial = new Tree();
					...
					for (int j = 0; j < boardBeforeMove.board.length; j++) {
						if (moveToMake.board[i][j] != boardBeforeMove.board[i][j]) {
							if (moveToMake.board[i][j] == 3 && boardBeforeMove.board[i][j] != 3) {
								newArrowcoord[0] = i;
								newArrowcoord[1] = j;
							} else if (boardBeforeMove.board[i][j] == 0 && moveToMake.board[i][j] == 2) {
			...
			/*
			 * ArrayList<Integer> QueenPosCurEnemey = (ArrayList<Integer>) msgDetails
			 * .get(AmazonsGameMessage.QUEEN_POS_CURR); ArrayList<Integer> QueenPosNewEnemey
			 * = (ArrayList<Integer>) msgDetails .get(AmazonsGameMessage.Queen_POS_NEXT);
			 * ArrayList<Integer> ArrowPosEnemey = (ArrayList<Integer>)
			 * msgDetails.get(AmazonsGameMessage.ARROW_POS);
			 * 
			 * //board.printBoard();
			 * 
			 * // Calculate Heuristic and print TerritoryHeuristic heur = new
			 * TerritoryHeuristic();
			 * TerritoryHeuristic.printHeuristic(heur.closestQueen(board)); // Update game
			 * board board.updateGameBoard(board, QueenPosCurEnemey, QueenPosNewEnemey,
			 * ArrowPosEnemey,true);
			 * 
			 * // Our positions to send
			 * 
			 * ArrayList<Integer> QueenPosCurSend = new ArrayList<>(); ArrayList<Integer>
			 * QueenPosNewSend = new ArrayList<>(); ArrayList<Integer> ArrowPosSend = new
			 * ArrayList<>();
			 * 
			 * QueenPosCurSend.add(4); // row [4,1] QueenPosCurSend.add(1); // col
			 * 
			 * QueenPosNewSend.add(5); QueenPosNewSend.add(2); // [5,2]
			 * 
			 * ArrowPosSend.add(5); ArrowPosSend.add(3);
			 * 
			 * // print the board after all the moves have been made
			 * board.updateGameBoard(board, QueenPosCurSend, QueenPosNewSend,
			 * ArrowPosSend,true); board.printBoard();
			 * 
			 * // Calculate Heuristic and print
			 * TerritoryHeuristic.printHeuristic(heur.closestQueen(board));
			 * 
			 * // Sending the positions gameClient.sendMoveMessage(QueenPosCurSend,
			 * QueenPosNewSend, ArrowPosSend);
			 * 
			 * this.gamegui.updateGameState(QueenPosCurSend, QueenPosNewSend, ArrowPosSend);
			 */t = 30;
