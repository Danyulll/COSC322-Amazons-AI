	...
	public Tree generatePartialGameTree(Board curr, boolean white) {
		...
		if (white) {
			...
			for (int i = 0; i < 4; i++) {
				...
				/*
				System.out.println("Queen1: " + whiteQueenLocs.get(0)[0] + "," + whiteQueenLocs.get(0)[1] + "\n"
						+ "Queen2: " + whiteQueenLocs.get(1)[0] + "," + whiteQueenLocs.get(1)[1] + "\n" + "Queen3: "
						+ whiteQueenLocs.get(2)[0] + "," + whiteQueenLocs.get(2)[1] + "\n" + "Queen4: "
						+ whiteQueenLocs.get(3)[0] + "," + whiteQueenLocs.get(3)[1] + "\n");*/
			...
			/*
			 * Queen Queen1 = new Queen(new Position(whiteQueenLocs.get(0)[0],
			 * whiteQueenLocs.get(0)[1])); System.out.println("Positiion of Queen 1: " +
			 * Queen1.getQueenCurrX() + "," + Queen1.getQueenCurrY()); // Queen Queen2 = new
			 * Queen(new Position(whiteQueenLocs.get(1)[0], // whiteQueenLocs.get(1)[1]));
			 * // Queen Queen3 = new Queen(new Position(whiteQueenLocs.get(2)[0], //
			 * whiteQueenLocs.get(2)[1])); // Queen Queen4 = new Queen(new
			 * Position(whiteQueenLocs.get(3)[0], // whiteQueenLocs.get(3)[1])); // get
			 * legal moves LegalMove moveGetter = new LegalMove(); ArrayList<Position>
			 * movesQueen1 = moveGetter.getLegalMove(Queen1, curr);
			 * System.out.println("Getting legal moves for Queen 1, ex: " +
			 * movesQueen1.get(0).getX() + "," + movesQueen1.get(0).getY()); //
			 ...
			 * //int counter = -20; for (Position position : movesQueen1) { System.out
			 * .println("For each move in the queen's move list generate a full list of possible arrow shots"
			 * ); ArrayList<Position> arrowMoves = moveGetter.getLegalMove(new
			 * Queen(position), curr); System.out.println("ex: " + arrowMoves.get(0).getX()
			 * + "," + arrowMoves.get(0).getY());
			 * 
			 * for (Position position2 : arrowMoves) { //counter++; System.out.
			 * println("For each arrow move position lets generate a board and add it to the tree"
			 * ); ArrayList<Integer> QueenPosCur = new ArrayList<>(); ArrayList<Integer>
			 * QueenPosNew = new ArrayList<>(); ArrayList<Integer> ArrowPos = new
			 * ArrayList<>();
			 * 
			 * QueenPosCur.add(Queen1.getQueenCurrX());
			 * QueenPosCur.add(Queen1.getQueenCurrY());
			 * 
			 * QueenPosNew.add(position.getX()); QueenPosNew.add(position.getY());
			 * 
			 * ArrowPos.add(position2.getX()); ArrowPos.add(position2.getY());
			 * 
			 * // clone the current board so we can add a updated version to the tree that
			 * // doesn't affect our current board
			 * System.out.println("curr before clone: "); curr.printBoard();
			 * System.out.println("tempbaord after clone but before update: "); Board
			 * tempBoard = (Board) curr.clone(); tempBoard.printBoard();
			 * 
			 * System.out.println("tempbaord after update");
			 * tempBoard.updateGameBoard(tempBoard, QueenPosCur, QueenPosNew, ArrowPos,
			 * false); tempBoard.printBoard();
			 * 
			 * System.out.println("Cur after updating tempboard (should be unchanged))");
			 * curr.printBoard(); //System.out.
			 * println("Find out what part is not cloning:\nBoards: currBoard- " +
			 * curr.board.toString() + "\ntempBaord- " + tempBoard.board.toString() +
			 * "\nWhite Pos curr- " + curr.WhitePos + "\n Temp whitepos- " +
			 * tempBoard.WhitePos + "\n Cur black pos- " + curr.BlackPos +
			 * "\n Temp Blackpos- " + tempBoard.BlackPos ); Node tempNode = new
			 * Node(this.root, 0); tempNode.setBoard(tempBoard);
			 * partial.root.addChild(tempNode);
			 * 
			 * //if(counter==1) //break;
			 * 
			 * } //counter++; System.out.println("tree generation done"); //if(counter==2)
			 * //break; }
			 */
